...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
	...
	if (mod == MOD_CRUSH) {
		...
		if ((targ->svflags & SVF_MONSTER) && targ->client) {
			if ((targ->client->zc.waitin_obj == inflictor && targ->client->zc.zcstate) || targ->groundentity == inflictor) {
	...
	if (take) {
		if ((targ->svflags & SVF_MONSTER) || (client)) {
			...
			if (client && (targ->svflags & SVF_MONSTER) && attacker) {
				...
				if (mod == MOD_RAILGUN || mod == MOD_BFG_LASER || mod == MOD_ROCKET || mod == MOD_BLASTER || mod == MOD_RIPPER || mod == MOD_HYPERBLASTER || mod == MOD_PHALANX) {
					if (attacker->client && (9 * random() < Bot[client->zc.botindex].param[BOP_REACTION]) && !client->zc.first_target) {
						if (!OnSameTeam(targ, attacker))
