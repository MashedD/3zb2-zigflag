...
void Cmd_Stats_f (edict_t *ent, bool check_other)
{
			...
			for (i = 0; i < num_cached_stats; i++) {
				if (Q_stricmp(cached_stats[i].name, name) == 0) {
					...
					for (i = FRAG_BLASTER; i <= FRAG_BFG; i++) {
						if (e->frags[i].atts == 0 && e->frags[i].kills == 0 && e->frags[i].deaths == 0)
							continue;
						int ac = e->frags[i].atts > 0 ? (int)(((float)e->frags[i].hits * 100.0f / (float)e->frags[i].atts) + 0.5f) : 0;
						if (ac > 100)
							ac = 100;
						gi.cprintf(ent, PRINT_HIGH, "%-16s %3d%% %5d/%-5d  %4d  %4d  %3d\n", weapon_names[i], ac, e->frags[i].hits, e->frags[i].atts, e->frags[i].kills, e->frags[i].deaths, e->frags[i].suicides);
						s_atts += e->frags[i].atts;
						s_hits += e->frags[i].hits;
						s_kills += e->frags[i].kills;
						s_deaths += e->frags[i].deaths;
	...
	for (i = FRAG_BLASTER; i <= FRAG_BFG; i++) {
		...
		gi.cprintf(ent, PRINT_HIGH, "%-16s %3d%% %5d/%-5d  %4d  %4d  %3d\n", weapon_names[i], acc, frags[i].hits, frags[i].atts, frags[i].kills, frags[i].deaths, frags[i].suicides);
	...
	if (level.intermissiontime && num_cached_stats > 0) {
		...
		for (i = 0; i < num_cached_stats - 1; i++) {
			for (j = i + 1; j < num_cached_stats; j++) {
				int k1 = 0, k2 = 0;
				for (x = 0; x < FRAG_TOTAL; x++)
					k1 += cached_stats[order[i]].frags[x].kills;
				for (x = 0; x < FRAG_TOTAL; x++)
					k2 += cached_stats[order[j]].frags[x].kills;
				if (k1 < k2) {
					int tmp = order[i];
					order[i] = order[j];
					order[j] = tmp;
		...
		for (i = 0; i < num_players - 1; i++) {
			for (j = i + 1; j < num_players; j++) {
				int k1 = 0, k2 = 0;
				for (x = 0; x < FRAG_TOTAL; x++)
					k1 += players[i]->client->resp.frags[x].kills;
				for (x = 0; x < FRAG_TOTAL; x++)
					k2 += players[j]->client->resp.frags[x].kills;
				if (k1 < k2) {
					edict_t *tmp = players[i];
					players[i] = players[j];
					players[j] = tmp;
				}
			}
