...
void BeginIntermission (edict_t *targ)
{
	...
	if (!deathmatch->value && (targ->map && targ->map[0] != '*')) { // go immediately to the next level
	...
	if (!ctf->value || zigmode->value != 1) {
		if (ent->client->zc.aiming == 1) {
			...
			if (ent->client->zc.lockon)
				ent->client->ps.stats[STAT_SIGHT_PIC] = gi.imageindex("zsight_l1");
			else
				ent->client->ps.stats[STAT_SIGHT_PIC] = gi.imageindex("zsight_l0");
		} else
