...
void NoAmmoWeaponChange (edict_t *ent)
{
	...
	if (ent->client->pers.inventory[ITEM_INDEX(Fdi_SLUGS /*FindItem("slugs")*/)] && ent->client->pers.inventory[ITEM_INDEX(Fdi_RAILGUN /*FindItem("railgun")*/)]) {
		item = Fdi_RAILGUN; //FindItem ("railgun");
				    //		return;
	}
	// RAFAEL
	else if (ent->client->pers.inventory[ITEM_INDEX(Fdi_MAGSLUGS /*FindItem ("mag slug")*/)] && ent->client->pers.inventory[ITEM_INDEX(Fdi_PHALANX /*FindItem ("phalanx")*/)]) {
		item = Fdi_PHALANX; //FindItem ("phalanx");
	}
	// RAFAEL
	else if (ent->client->pers.inventory[ITEM_INDEX(Fdi_CELLS /*FindItem ("cells")*/)] && ent->client->pers.inventory[ITEM_INDEX(Fdi_BOOMER /*FindItem ("ionripper")*/)]) {
		item = Fdi_BOOMER; //FindItem ("ionrippergun");
	}

	else if (ent->client->pers.inventory[ITEM_INDEX(Fdi_CELLS /*FindItem("cells")*/)] && ent->client->pers.inventory[ITEM_INDEX(Fdi_HYPERBLASTER /*FindItem("hyperblaster")*/)]) {
		item = Fdi_HYPERBLASTER; //FindItem ("hyperblaster");
					 //		return;
	} else if (ent->client->pers.inventory[ITEM_INDEX(Fdi_BULLETS /*FindItem("bullets")*/)] && ent->client->pers.inventory[ITEM_INDEX(Fdi_CHAINGUN /*FindItem("chaingun")*/)]) {
		item = Fdi_CHAINGUN; //FindItem ("chaingun");
				     //		return;
	} else if (ent->client->pers.inventory[ITEM_INDEX(Fdi_BULLETS /*FindItem("bullets")*/)] && ent->client->pers.inventory[ITEM_INDEX(Fdi_MACHINEGUN /*FindItem("machinegun")*/)]) {
		item = Fdi_MACHINEGUN; //FindItem ("machinegun");
				       //		return;
	} else if (ent->client->pers.inventory[ITEM_INDEX(Fdi_SHELLS /*FindItem("shells")*/)] > 1 && ent->client->pers.inventory[ITEM_INDEX(Fdi_SUPERSHOTGUN /*FindItem("super shotgun")*/)]) {
		item = Fdi_SUPERSHOTGUN; //FindItem ("super shotgun");
					 //		return;
	} else if (ent->client->pers.inventory[ITEM_INDEX(Fdi_SHELLS /*FindItem("shells")*/)] && ent->client->pers.inventory[ITEM_INDEX(Fdi_SHOTGUN /*FindItem("shotgun")*/)]) {
...
void Weapon_LockonRocketLauncher_Fire (edict_t *ent)
{
	...
	if (ent->client->buttons & BUTTON_ATTACK) {
		...
		if (ent->client->zc.aiming == 0) {
			...
			if (ent->client->zc.distance < 10 || ent->client->zc.distance > 90)
...
void Weapon_RocketLauncher (edict_t *ent)
{
	...
	if (!(ent->client->buttons & BUTTON_ATTACK))
		ent->client->zc.aiming = 0; //アクティベート0
...
void weapon_railgun_fire (edict_t *ent)
{
	...
	if (instagib && instagib->value) {
		damage = 10000;
		kick = 200;
	} else if (deathmatch->value) { // normal damage is too extreme in dm
		damage = 100;
		kick = 200;
	} else {
		damage = 150;
