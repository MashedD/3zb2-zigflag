...
void SelectSpawnPoint (edict_t *ent, vec3_t origin, vec3_t angles)
{
	...
	if ((ent)->client && ((ent)->client->zc.botindex || (ent)->client->zc.routeindex > 0))
		origin[2] += 14;
	else
		origin[2] += 9;
...
void CopyToBodyQue (edict_t *ent)
{
	...
	//強引にフレームセット
	if (body->s.modelindex == skullindex || body->s.modelindex == headindex)
		body->s.frame = 0;
...
bool TraceX (edict_t *ent, vec3_t p2)
{
	...
	if (rs_trace.fraction == 1.0 && !rs_trace.allsolid && !rs_trace.startsolid)
		return true;
...
void ClientThink (edict_t *ent, usercmd_t *ucmd)
{
	...
	//route nodeput
	j = 0;
	if (ent->client->ctf_grapple && ent->client->ctf_grapplestate > CTF_GRAPPLE_STATE_FLY)
		j = 1;
	...
	if (!j && chedit->value && CurrentIndex < MAXNODES && !ent->deadflag && ent == &g_edicts[1]) {
		...
		if (!Route[CurrentIndex].index) {
			...
			i = true;
			if (ent->groundentity) {
				if (!Q_stricmp(ent->groundentity->classname, "func_train") || !Q_stricmp(ent->groundentity->classname, "func_plat") || !Q_stricmp(ent->groundentity->classname, "func_door"))
					i = false;
			}
		...
		} else if (((/*ent->velocity[2] > 10 &&*/ !ent->groundentity && wasground == true) || (/*ent->velocity[2] < -0.5 &&*/ ent->groundentity && wasground == false)) && Route[CurrentIndex - 1].state <= GRS_ITEMS) {
			j = false;
			k = true; //false;
			VectorCopy(ent->s.old_origin, v);
			v[2] -= 2;
			...
			if (old_ground) {
				if (!Q_stricmp(old_ground->classname, "func_train") || !Q_stricmp(old_ground->classname, "func_plat") || !Q_stricmp(old_ground->classname, "func_door"))
					k = false;
			}
	...
	if (level.intermissiontime) {
		...
		if (level.time > level.intermissiontime + delay && (ucmd->buttons & BUTTON_ATTACK))
