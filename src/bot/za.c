...
static float Bot_TargetPriority (edict_t *ent, edict_t *target, int own_flag_index, int *reason)
{
	vec3_t delta;
	float distance;
	...
	if (target->client->zc.first_target == ent) {
		policy.attacks_me = true;
		if (reason)
			*reason = BOT_REASON_DIRECT_ATTACKER;
	}
	...
	if (distance < 220.0f && ent->health < 50)
		policy.close_while_vulnerable = true;

	return BotPolicy_TargetScore(&policy);
}

static float Bot_ItemPriority (edict_t *ent, edict_t *item, float distance, int *reason)
{
	const char *classname = item->classname ? item->classname : "";
	bot_policy_item_t policy = {0};
	gitem_t *pickup_item = item->item;
	int pickup = Bot[ent->client->zc.botindex].param[BOP_PICKUP];

	if (reason)
		*reason = BOT_REASON_ITEM_NEED;
	if (pickup > 9)
		pickup = 9;

	policy.distance = distance;
	policy.pickup_skill = pickup;
	policy.kind = BOT_ITEM_ROAM;

	if (pickup_item && (pickup_item->flags & IT_ARMOR)) {
		int armor_index = ArmorIndex(ent);
		int armor = armor_index ? ent->client->pers.inventory[armor_index] : 0;
		gitem_armor_t *armor_info = pickup_item->info;
		policy.kind = BOT_ITEM_ARMOR;
		policy.need = armor_info && armor_info->max_count ?
			(float)(armor_info->max_count - armor) / armor_info->max_count : 0.5f;
		if (policy.need < 0)
			policy.need = 0;
		policy.effective_gain = armor_info ? armor_info->base_count : pickup_item->quantity;
	} else if (pickup_item && (pickup_item->flags & IT_WEAPON)) {
		int weapon = Get_KindWeapon(pickup_item);
		int current = Get_KindWeapon(ent->client->pers.weapon);
		policy.kind = BOT_ITEM_WEAPON;
		policy.weapon_upgrade = weapon > current;
		policy.preferred_weapon = weapon == Bot[ent->client->zc.botindex].param[BOP_PRIWEP];
		policy.dropped = (item->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM)) != 0;
		if (reason)
			*reason = BOT_REASON_ITEM_UPGRADE;
	} else if (pickup_item && (pickup_item->flags & IT_AMMO)) {
		int inventory = ent->client->pers.inventory[ITEM_INDEX(pickup_item)];
		int maximum = 0;
		policy.kind = BOT_ITEM_AMMO;
		if (pickup_item == Fdi_SHELLS) maximum = ent->client->pers.max_shells;
		else if (pickup_item == Fdi_BULLETS) maximum = ent->client->pers.max_bullets;
		else if (pickup_item == Fdi_GRENADES) maximum = ent->client->pers.max_grenades;
		else if (pickup_item == Fdi_ROCKETS) maximum = ent->client->pers.max_rockets;
		else if (pickup_item == Fdi_CELLS) maximum = ent->client->pers.max_cells;
		else if (pickup_item == Fdi_SLUGS) maximum = ent->client->pers.max_slugs;
		if (maximum > 0) {
			policy.need = (float)(maximum - inventory) / maximum;
			policy.effective_gain = pickup_item->quantity < maximum - inventory ?
				pickup_item->quantity : maximum - inventory;
		}
	} else if (!strncmp(classname, "item_flag", 9) || !strncmp(classname, "ZB", 2)) {
		policy.kind = BOT_ITEM_OBJECTIVE;
		if (reason)
			*reason = BOT_REASON_OBJECTIVE_CARRIER;
	} else if (pickup_item && (pickup_item->flags & (IT_POWERUP | IT_TECH))) {
		policy.kind = BOT_ITEM_POWERUP;
	} else if (!strncmp(classname, "item_health", 11)) {
		int cap = strstr(classname, "mega") ? 200 : 100;
		policy.kind = BOT_ITEM_HEALTH;
		policy.need = ent->health < cap ? (float)(cap - ent->health) / cap : 0;
		policy.effective_gain = item->count > 0 ? item->count : 10;
	}

	return BotPolicy_ItemScore(&policy);
}

static bool Bot_CanDetourForItems (edict_t *ent)
{
	zgcl_t *zc = &ent->client->zc;

	if (zc->first_target && ent->health >= 35)
		return false;
	if (Bot_IsObjectiveCTF() &&
	    (zc->ctfstate == CTFS_CARRIER || zc->ctfstate == CTFS_SUPPORTER))
		return false;
...
int Bot_SearchEnemy (edict_t *ent)
{
	...
	edict_t *target, *trent, *old_target;
	trace_t rs_trace;

	int i, j, k, own_flag_index;
	int foundedenemy;
	int selected_reason = BOT_REASON_NONE;
	float best_priority;
	...
	if (target) {
		best_priority = Bot_TargetPriority(ent, target, own_flag_index, &selected_reason) +
		                140.0f + (9 - Bot[zc->botindex].param[BOP_REACTION]) * 12.0f;
	}
	...
	for (i = 1; i <= maxclients->value; i++) {
		...
		if (entcln[0] == 'p' && trent->movetype != MOVETYPE_NOCLIP) {
			if (Bot_traceS(ent, trent)) {
				...
				if (!Bot_IsObjectiveCTF() && Bot_IsTeamMode() && OnSameTeam(ent, trent)) {
					...
					if (trmin[2] < JumpMax && VectorLength(trmin) < 400 &&
						...
						if (zflag_item && trent->client->pers.inventory[ITEM_INDEX(zflag_item)]) {
							...
							if ((9 * random()) < yaw) {
								...
								if (trent->client->zc.tmplstate == TMS_LEADER) {
									...
									if (ent->client->zc.tmplstate == TMS_LEADER || random() < 0.5) {
										ent->client->zc.tmplstate = TMS_LEADER;
										ent->client->zc.followmate = NULL;
										trent->client->zc.tmplstate = TMS_FOLLOWER;
										trent->client->zc.followmate = ent;
						...
						if (ent->client->zc.followmate)
							ent->client->zc.matelock = level.time + 5.0f;
					}
				} else if (Bot_IsObjectiveCTF() && ent->client->resp.ctf_team == trent->client->resp.ctf_team) {
					//have flag
					if (trent->client->pers.inventory[k]) {
						/* Support roles are assigned centrally so the whole team does
						 * not abandon offense when the carrier passes nearby. */
						// if carrier have enemy
						if (Bot_ValidEnemy(ent, trent->client->zc.first_target) &&
						    Bot_traceS(ent, trent->client->zc.first_target)) {
							float priority = Bot_TargetPriority(ent, trent->client->zc.first_target,
							                                    own_flag_index, NULL) + 700.0f;
							if (priority > best_priority) {
								target = trent->client->zc.first_target;
								best_priority = priority;
								selected_reason = BOT_REASON_TEAM_ASSIST;
							}
						}
					...
					foundedenemy++;
					{
						...
						{
							int reason;
							priority = Bot_TargetPriority(ent, trent, own_flag_index, &reason);
						...
						if (Bot[ent->client->zc.botindex].param[BOP_NOISECHK]) {
							if (trent->mynoise->teleport_time >= (level.time - FRAMETIME)) {
								...
								if (VectorLength(trmin) < 300) {
									...
									if ((9 * random()) < pitch) {
										int reason;
										float priority = Bot_TargetPriority(ent, trent, own_flag_index, &reason) - 250.0f;
										if (priority > best_priority) {
											target = trent;
											best_priority = priority;
											selected_reason = reason;
							...
							if (target == NULL && trent->mynoise2->teleport_time >= (level.time - FRAMETIME)) {
								...
								if (VectorLength(trmin) < 100) {
									...
									if ((9 * random()) < pitch) {
										int reason;
										float priority = Bot_TargetPriority(ent, trent, own_flag_index, &reason) - 300.0f;
										if (priority > best_priority) {
											target = trent;
											best_priority = priority;
											selected_reason = reason;
			...
			//音のみで場所を判断
				...
				if (Bot[ent->client->zc.botindex].param[BOP_NOISECHK] && Bot[ent->client->zc.botindex].param[BOP_ESTIMATE] && !tmpflg && trent->mynoise) {
					if (trent->mynoise->teleport_time >= (level.time - FRAMETIME)) {
						...
						if (VectorLength(trmin) < 500) {
							...
							if (rs_trace.fraction == 1.0 && (9 * random()) < pitch) {
								target = trent;
								best_priority = Bot_TargetPriority(ent, trent, own_flag_index,
								                                   &selected_reason) - 350.0f;
	...
	/* Do not flick between ordinary targets. Objective carriers and direct
	 * attackers may always break the short skill-scaled target lock. */
	if (target && old_target && target != old_target && Bot_ValidEnemy(ent, old_target) &&
	    Bot_trace(ent, old_target)) {
		float old_score = Bot_TargetPriority(ent, old_target, own_flag_index, NULL);
		bool urgent = selected_reason == BOT_REASON_OBJECTIVE_CARRIER ||
		              selected_reason == BOT_REASON_DIRECT_ATTACKER;
		if (!BotPolicy_ShouldSwitchTarget(old_score, best_priority,
		                                 level.time < zc->target_switch_time, urgent)) {
			target = old_target;
			best_priority = Bot_TargetPriority(ent, target, own_flag_index, &selected_reason);
		}
	}
	...
	if (target) {
		...
		if (target != old_target) {
			...
			zc->target_acquire_time = level.time + 0.45f - reaction * 0.035f + random() * 0.08f;
		}
...
void Bot_SearchItems (edict_t *ent)
{
	...
	edict_t *target, *trent, *best_item;
	...
	float best_item_score;
	int best_item_reason;
	...
	zc = &ent->client->zc;
	best_item = NULL;
	best_item_score = -999999.0f;
	best_item_reason = BOT_REASON_NONE;
	...
	if (ctf->value) {
		if (ent->moveinfo.state == SUPPORTER)
			...
			if (ent->moveinfo.state == CARRIER && ent->target_ent->classname[6] == 'F')
				j = -1;
	...
	if (trent != NULL && !j) {
		...
		if (trent->classname[0] == 'w')
			...
			if (trent->classname[5] == 'q' || trent->classname[5] == 'f' || trent->classname[5] == 't' || trent->classname[5] == 'i')
				j = -1;
		} else if ((trent->classname[0] == 'p' && x > 0.2) || ent->client->ctf_grapple != NULL)
			j = -1;
	}
			...
			if (trent->solid != SOLID_TRIGGER) {
				if (ent->client->weaponstate == WEAPON_READY && !ent->client->zc.route_trace) {
					if (entcln) {
						...
						{
							if (trent->classname[5] == 'b' && trent->monsterinfo.attack_finished > level.time)
								continue;
				...
				if (entcln[0] == 'i') {
					if (entcln[5] == 'h') {
						if (Q_stricmp(entcln, "item_health") == 0) {
							if (ent->health < 100 && (!pickup_pri || yaw < 96))
								target = trent;
						} else if (entcln[12] == 's') { //item_health_small
							if (fabs(trent->s.origin[2] - ent->s.origin[2] + 8) > JumpMax)
								continue;
							target = trent;
					...
					{
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_BULLETS /*FindItem("Bullets")*/)] < ent->client->pers.max_bullets)
							target = trent;
					} else if (entcln[5] == 's' && entcln[6] == 'h') //ammo_shells
					{
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_SHELLS /*FindItem("Shells")*/)] < ent->client->pers.max_shells)
							target = trent;
					} else if (entcln[5] == 'g') //ammo_grenades
					{
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_GRENADES /*FindItem("Grenades")*/)] < ent->client->pers.max_grenades)
							target = trent;
					} else if (entcln[5] == 'r') //ammo_rockets
					{
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_ROCKETS /*FindItem("Rockets")*/)] < ent->client->pers.max_rockets)
							target = trent;
					} else if (entcln[5] == 's') //ammo_slugs
					{
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_SLUGS /*FindItem("Slugs")*/)] < ent->client->pers.max_slugs)
							target = trent;
					} else if (entcln[5] == 'c') //ammo_cells
					{
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_CELLS /*FindItem("Cells")*/)] < ent->client->pers.max_cells)
							target = trent;
					} else if (entcln[5] == 'm') //ammo_magslug
					{
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_MAGSLUGS /*FindItem("Mag Slug")*/)] < ent->client->pers.max_magslug)
							target = trent;
					} else if (entcln[5] == 't') //ammo_trap
					{
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_TRAP /*FindItem("Trap")*/)] < ent->client->pers.max_trap)
							target = trent;
					}
				} else if (entcln[0] == 'w') {
					...
					{
						if (!wstayf || (wstayf && (!ent->client->pers.inventory[ITEM_INDEX(Fdi_SUPERSHOTGUN /*FindItem("Super Shotgun")*/)] || trent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM))))
							target = trent;
					} else if (entcln[7] == 'm') //weapon_machinegun
					{
						if (!wstayf || (wstayf && (!ent->client->pers.inventory[ITEM_INDEX(Fdi_MACHINEGUN /*FindItem("Machinegun")*/)] || trent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM))))
							target = trent;
					} else if (entcln[7] == 'c') //weapon_chaingun
					{
						if (!wstayf || (wstayf && (!ent->client->pers.inventory[ITEM_INDEX(Fdi_CHAINGUN /*FindItem("Chaingun")*/)] || trent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM))))
							target = trent;
					} else if (entcln[7] == 'g') //weapon_grenadelauncher
					{
						if (!wstayf || (wstayf && (!ent->client->pers.inventory[ITEM_INDEX(Fdi_GRENADELAUNCHER /*FindItem("Grenade Launcher")*/)] || trent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM))))
							target = trent;
					...
					{
						if (!wstayf || (wstayf && (!ent->client->pers.inventory[ITEM_INDEX(Fdi_HYPERBLASTER /*FindItem("HyperBlaster")*/)] || trent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM))))
							target = trent;
					} else if (entcln[7] == 'r') //weapon_railgun
					{
						if (!wstayf || (wstayf && (!ent->client->pers.inventory[ITEM_INDEX(Fdi_RAILGUN /*FindItem("Railgun")*/)] || trent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM))))
							target = trent;
					} else if (entcln[7] == 'b' && entcln[8] == 'o') //weapon_boomer
					{
						if (!wstayf || (wstayf && (!ent->client->pers.inventory[ITEM_INDEX(Fdi_BOOMER /*FindItem("Ionripper")*/)] || trent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM))))
							target = trent;
					} else if (entcln[7] == 'p') //weapon_phalanx
					{
						if (!wstayf || (wstayf && (!ent->client->pers.inventory[ITEM_INDEX(Fdi_PHALANX /*FindItem("Phalanx")*/)] || trent->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM))))
							target = trent;
					...
					if (entcln[2] == 'n') {
						...
						if (ent->client->zc.routeindex < CurrentIndex) {
							if (entcln[6] == 'X' && Route[ent->client->zc.routeindex].state == GRS_ONTRAIN)
								target = trent;
							else if (entcln[6] == '2' && Route[ent->client->zc.routeindex].state == GRS_PUSHBUTTON && Route[ent->client->zc.routeindex].ent == trent)
								target = trent;
							else
								target = NULL;
						}
...
*/
			if (target != NULL) {
				...
				if (conts & CONTENTS_LAVA)
					target = NULL;
				else if ((conts & CONTENTS_SLIME) && ent->client->enviro_framenum <= level.framenum)
					target = NULL;
			}
			if (target != NULL && !ctf->value) {
			...
			if (target != NULL) {
				int reason;
				float score = Bot_ItemPriority(ent, target, yaw, &reason);
				if (score > best_item_score) {
					best_item = target;
					best_item_score = score;
					best_item_reason = reason;
				}
			}
			target = NULL;
		}
...
int Bot_moveT (edict_t *ent, float ryaw, vec3_t pos, float dist, float *bottom)
{
	...
	if (0 /*!(ent->client->ps.pmove.pm_flags & PMF_DUCKED)
		&& (ent->client->zc.n_duckedtime < FRAMETIME * 10 && !ent->client->zc.route_trace)*/
	)
		trmax[2] = 31;
	//	else if(ent->waterlevel && !ent->groundentity) trmax[2] = 32;
	else if (ent->client->zc.route_trace && !(ent->client->ps.pmove.pm_flags & PMF_DUCKED) && ent->waterlevel < 2) {
	...
	if (rs_trace.allsolid || rs_trace.startsolid || rs_trace.fraction != 1.0) //前には進めない場合
	{
		...
		*bottom = rs_trace.endpos[2] - ent->s.origin[2];
		...
		if (!ent->client->zc.route_trace) {
			...
			if (rs_trace.plane.normal[2] < 0.7 && (!ent->client->zc.waterstate && ent->groundentity))
				return false;
		} else {
			Get_RouteOrigin(ent->client->zc.routeindex, v);
	...
	} else //進めたとしても落ちたくない時のためのチェック
	{
		...
		*bottom = rs_trace.endpos[2] - ent->s.origin[2];
		...
		if (0 /*!ent->client->zc.route_trace*/) {
			if (rs_trace.plane.normal[2] > 0 && rs_trace.plane.normal[2] < 0.7)
		...
		if (!ent->waterlevel && (ent->client->ps.pmove.pm_flags & PMF_DUCKED) && ent->groundentity == NULL && ent->velocity[2] > 10 && trmax[2] == 4)
			return 2;
...
void Set_Combatstate (edict_t *ent, int foundedenemy)
{
	...
	if (client->zc.target_visible)
		...
		VectorSubtract(client->zc.target_last_seen_pos, ent->s.origin, v);
...
int Bot_AssessThreat (edict_t *ent, edict_t *target, int foundedenemy, float distance)
{
	...
	int myArmor = ent->client->pers.inventory[ITEM_INDEX(FindItem("Jacket Armor"))] + ent->client->pers.inventory[ITEM_INDEX(FindItem("Combat Armor"))] + ent->client->pers.inventory[ITEM_INDEX(FindItem("Body Armor"))];
...
bool Bot_Jump (edict_t *ent, vec3_t pos, float dist)
{
	...
	if (!Bot_moveT(ent, yaw, temppos, dist, &bottom))
		return false;
	if (bottom > -JumpMax)
		return false;
...
void Get_WaterState (edict_t *ent)
{
	...
	if (ent->waterlevel) {
		...
		if (rs_trace.allsolid || rs_trace.startsolid || (/*x >= 4 &&*/ x < 4.0))
...
void Search_NearlyPod (edict_t *ent)
{
	...
	if ((ent->client->zc.routeindex + 1) < CurrentIndex) {
		...
		if (TraceX(ent, v)) {
			...
			if (VectorLength(v1) < VectorLength(v2) && x <= JumpMax && Route[ent->client->zc.routeindex].state <= GRS_ONROTATE) {
...
static const char *Bot_GoalName (int goal)
{
	switch (goal) {
		case BOT_GOAL_ENEMY: return "enemy";
		case BOT_GOAL_ITEM: return "item";
		case BOT_GOAL_OBJECTIVE: return "objective";
		case BOT_GOAL_FOLLOW: return "follow";
		case BOT_GOAL_ROUTE_RECOVERY: return "route-recovery";
		default: return "none";
	}
}

static const char *Bot_ReasonName (int reason)
{
	switch (reason) {
		case BOT_REASON_NEAREST_THREAT: return "nearest-threat";
		case BOT_REASON_DIRECT_ATTACKER: return "direct-attacker";
		case BOT_REASON_OBJECTIVE_CARRIER: return "objective-carrier";
		case BOT_REASON_TEAM_ASSIST: return "team-assist";
		case BOT_REASON_WEAK_ENEMY: return "weak-enemy";
		case BOT_REASON_ITEM_NEED: return "item-need";
		case BOT_REASON_ITEM_UPGRADE: return "item-upgrade";
		case BOT_REASON_ROUTE_STUCK: return "route-stuck";
		default: return "none";
	}
}

static void Bot_DebugAI (edict_t *ent)
{
	zgcl_t *zc = &ent->client->zc;
	int slot = ent - g_edicts;
	const char *target = "-";
	const char *weapon = ent->client->pers.weapon ? ent->client->pers.weapon->pickup_name : "-";

	if (!bot_debug_ai || (int)bot_debug_ai->value != slot || level.time < zc->ai_debug_time)
		return;
	if (zc->first_target && zc->first_target->inuse && zc->first_target->client)
		target = zc->first_target->client->pers.netname;
	else if (zc->second_target && zc->second_target->inuse && zc->second_target->classname)
		target = zc->second_target->classname;
	if (!weapon)
		weapon = "-";
	...
	zc->ai_debug_time = level.time + 1.0f;
}

static bool Bot_RouteWaitExempt (edict_t *ent)
{
	zgcl_t *zc = &ent->client->zc;

	if (chedit->value || (zc->zcstate & (STS_WAITSMASK | STS_LADDERUP)) ||
	    ent->client->ctf_grapple)
		return true;
	if (zc->push_time > 0 && zc->push_time + BOT_PUSH_IDLE_TIME_GROUND > level.time)
		return true;
	if (ent->groundentity && ent->groundentity->classname &&
...
static void Bot_RecoverRoute (edict_t *ent)
{
	zgcl_t *zc = &ent->client->zc;
	vec3_t delta, end, forward, down;
	trace_t tr, floor;
	float moved;
	int i, best = -1;
	float best_score = BOT_POLICY_REJECTED;

	if (!zc->route_trace || zc->routeindex < 0 || zc->routeindex >= CurrentIndex ||
	    Bot_RouteWaitExempt(ent)) {
		VectorCopy(ent->s.origin, zc->route_progress_origin);
		zc->route_progress_index = zc->routeindex;
		zc->route_progress_time = level.time;
		return;
	}

	VectorSubtract(ent->s.origin, zc->route_progress_origin, delta);
	moved = VectorLength(delta);
	if (!zc->route_progress_time || zc->route_progress_index != zc->routeindex || moved >= 48.0f) {
		VectorCopy(ent->s.origin, zc->route_progress_origin);
		zc->route_progress_index = zc->routeindex;
		zc->route_progress_time = level.time;
		zc->route_recovery_attempts = 0;
		if (zc->ai_goal == BOT_GOAL_ROUTE_RECOVERY) {
			zc->ai_goal = BOT_GOAL_NONE;
			zc->ai_reason = BOT_REASON_NONE;
		}
		return;
	}
	if (level.time < zc->route_recovery_time || level.time - zc->route_progress_time < 1.5f)
		return;

	zc->ai_goal = BOT_GOAL_ROUTE_RECOVERY;
	zc->ai_reason = BOT_REASON_ROUTE_STUCK;
	zc->ai_goal_score = 0;
	zc->route_recovery_attempts++;
	zc->route_recovery_time = level.time + 0.75f;

	/* First try a locally safe sidestep. This preserves the active route and
	 * handles transient player obstruction without inventing navigation data. */
	if (zc->route_recovery_attempts == 1) {
		float side = (random() < 0.5f) ? -90.0f : 90.0f;
		float recovery_yaw = zc->moveyaw + side;
		vec3_t angles = { 0, recovery_yaw, 0 };
		AngleVectors(angles, forward, NULL, NULL);
		VectorMA(ent->s.origin, 64.0f, forward, end);
		tr = gi.trace(ent->s.origin, ent->mins, ent->maxs, end, ent, MASK_BOTSOLID);
		VectorCopy(tr.endpos, down);
		down[2] -= 64.0f;
		floor = gi.trace(tr.endpos, ent->mins, ent->maxs, down, ent, MASK_BOTGROUND);
		if (tr.fraction < 0.5f || floor.fraction == 1.0f ||
		    (gi.pointcontents(floor.endpos) & (CONTENTS_LAVA | CONTENTS_SLIME))) {
			recovery_yaw = zc->moveyaw - side;
			angles[YAW] = recovery_yaw;
			AngleVectors(angles, forward, NULL, NULL);
			VectorMA(ent->s.origin, 64.0f, forward, end);
			tr = gi.trace(ent->s.origin, ent->mins, ent->maxs, end, ent, MASK_BOTSOLID);
			VectorCopy(tr.endpos, down);
		...
		if (tr.fraction >= 0.5f && floor.fraction < 1.0f &&
		    !(gi.pointcontents(floor.endpos) & (CONTENTS_LAVA | CONTENTS_SLIME))) {
			zc->moveyaw = recovery_yaw;
			trace_priority = TRP_MOVEKEEP;
			if (ent->groundentity && Bot[zc->botindex].param[BOP_COMBATSKILL] >= 6 &&
	...
	/* Reacquire the nearest visible pod in a bounded window around the failed
	 * route index. If none is reachable, release route tracing briefly so the
	 * normal route search can start from a clean state. */
	for (i = zc->routeindex - 24; i <= zc->routeindex + 24; i++) {
		vec3_t point;
		float distance, score;
		bot_policy_route_t candidate = {0};
		int link;
		if (i < 0 || i >= CurrentIndex || i == zc->routeindex || Route[i].state > GRS_ITEMS)
			continue;
		Get_RouteOrigin(i, point);
		VectorSubtract(point, ent->s.origin, delta);
		distance = VectorLength(delta);
		if (fabs(delta[2]) > JumpMax || !TraceX(ent, point))
			continue;
		candidate.distance = distance;
		candidate.forward = i > zc->routeindex;
		candidate.backtrack = i < zc->routeindex;
		for (link = 0; link < MAXLINKPOD - (ctf->value != 0); link++)
			if (Route[zc->routeindex].linkpod[link] == i)
				candidate.linked = true;
		if (ctf->value && Route[i].state < GRS_ITEMS &&
		    Route[zc->routeindex].state < GRS_ITEMS)
			candidate.objective_match =
				(Route[i].linkpod[MAXLINKPOD - 1] & CTF_FLAG2_FLAG) ==
				(Route[zc->routeindex].linkpod[MAXLINKPOD - 1] & CTF_FLAG2_FLAG);
		score = BotPolicy_RouteScore(&candidate);
		if (score <= best_score)
			continue;
		best = i;
		best_score = score;
	...
	if (best >= 0) {
		...
		zc->route_trace = false;
		zc->rt_releasetime = level.time + 0.35f;
	}
	VectorCopy(ent->s.origin, zc->route_progress_origin);
	zc->route_progress_index = zc->routeindex;
	zc->route_progress_time = level.time;
	zc->route_recovery_time = level.time + 2.0f;
}
...
void Bots_Move_NORM (edict_t *ent)
{
	...
	if (!zc->havetarget && zc->route_trace) {
		...
		if ((j && !ent->client->pers.inventory[j]) || k) {
			if (!k)
				it = &itemlist[j];
			if (zc->targetindex < zc->routeindex || zc->targetindex >= CurrentIndex)
	...
	if (ent->client->pers.inventory[ITEM_INDEX(it)] && ent->client->pers.inventory[i] > 0)
		canrocj = true;
	else
		canrocj = false;
	...
	} else if (ent->velocity[2] > 10 && ent->groundentity == NULL && !(zc->zcstate & STS_SJMASK)) {
...
DCHCANC: //しゃがみっぱなし
	...
	if (level.time < zc->target_acquire_time)
		ent->client->buttons &= ~BUTTON_ATTACK;

	// TDM weapon sharing - drop extra weapons for nearby teammates
	if ((tdm->value || ctf->value) && !zc->first_target &&
	    level.time >= zc->donation_time) {
		int tdm_j, tdm_k, tdm_idx, tdm_dropped;
		edict_t *tdm_team;

		for (tdm_j = 1; tdm_j <= maxclients->value; tdm_j++) {
			tdm_team = &g_edicts[tdm_j];
			if (!tdm_team->inuse || ent == tdm_team || tdm_team->deadflag)
				continue;
			if (!tdm_team->client)
				continue;
			if (tdm_team->svflags & SVF_MONSTER)
				continue;
			...
			if (tdm_dropped)
				zc->donation_time = level.time + 8.0f;
			if (tdm_dropped)
				break;
		}
	}
		...
		if (zc->second_target->solid != SOLID_TRIGGER || !zc->second_target->inuse) {
			...
			if (temppos[2] > 32) {
				if (!canrocj) {
					if (x < 0 || x > 32)
						k = true;
					else if (!Bot_trace2(ent, zc->second_target->moveinfo.start_origin))
						k = true;
				} else {
					if (temppos[2] > 300)
						k = true;
	...
	if (zc->route_trace) {
		...
		if (zc->routeindex > 0) {
			...
			//撃てGrapple
			else if (Route[zc->routeindex - 1].state == GRS_GRAPSHOT && ent->client->ctf_grapple == NULL && zc->first_target == NULL) {
				item = Fdi_GRAPPLE;
				; //FindItem("Grapple");
					...
					if (Route[zc->routeindex].state != GRS_GRAPRELEASE) {
						...
						if ((zc->routeindex - i) > 0) {
							for (j = 0; (zc->routeindex - i + j) < CurrentIndex; j++) {
								if (Route[zc->routeindex - i + j].state == GRS_GRAPRELEASE)
									break;
							}
				...
				} else if (ent->client->ctf_grapplestate == CTF_GRAPPLE_STATE_HANG && Route[zc->routeindex].state != GRS_GRAPRELEASE) {
					item = Fdi_GRAPPLE; //FindItem("Grapple");
					item->use(ent, item);
					//					ent->client->pers.weapon = item;
					//					ent->s.sound = 0;
			...
			else if (Route[zc->routeindex - 1].state == GRS_GRAPHOOK && ent->client->ctf_grapple) {
			...
			if (Route[zc->routeindex].state == GRS_GRAPRELEASE && ent->client->ctf_grapple) {
				k = 0;
				if (ent->client->ctf_grapplestate == CTF_GRAPPLE_STATE_FLY)
					trace_priority = TRP_ALLKEEP;
				...
				if (k) {
					if (zc->routeindex < CurrentIndex) {
						if (Route[zc->routeindex].state == GRS_GRAPSHOT) {
							if (1 /*TraceX(ent,Route[zc->routeindex + 1].Tcourner)*/)
								zc->routeindex++;
						}
					}
				}
		...
		if (zc->zcstate & STS_WAITSMASK)
			zc->rt_locktime = level.time + FRAMETIME * POD_LOCKFRAME;
		else if (Route[zc->routeindex].state <= GRS_ITEMS && (x > JumpMax && !zc->waterstate) && !(zc->zcstate & STS_LADDERUP)) {
...
//				gi.bprintf(PRINT_HIGH,"shit1");
			...
			if (Route[zc->routeindex].state > GRS_ITEMS && Route[zc->routeindex].state <= GRS_ONPLAT) {
	...
	//--------------------------------------------------------------------------------------
	//search for items
	if (!chedit->value && !(instagib && instagib->value) &&
	    zc->second_target == NULL && Bot_CanDetourForItems(ent)) {
	...
	if (zc->second_target != NULL) {
		if ((zc->second_target->s.origin[2] - ent->s.origin[2]) > 32 && !ent->waterlevel) {
			...
			VectorSubtract(zc->second_target->s.origin, ent->s.origin, temppos);
			if (ent->waterlevel && !ent->groundentity && trace_priority < TRP_ANGLEKEEP)
			...
			if (!ent->groundentity && !ent->waterlevel && trace_priority < TRP_ANGLEKEEP) {
				...
				if (!k) {
					if (trace_priority < TRP_MOVEKEEP)
						zc->moveyaw = Get_yaw(temppos); //set the movin' yaw
					if (trace_priority < TRP_ANGLEKEEP)
						ent->s.angles[YAW] = zc->moveyaw;
...
*/
	...
	if (Bot_IsTeamMode()) {
		...
		if (j /*zc->tmplstate == TMS_FOLLOWER && zc->second_target == NULL*/) {
			if (zc->followmate) {
				zc->ai_goal = BOT_GOAL_FOLLOW;
				zc->ai_reason = BOT_REASON_TEAM_ASSIST;
				zc->ai_goal_score = 0;
				...
				if (!zc->followmate->inuse || zc->followmate->deadflag || zc->matelock <= level.time) {
					...
					if (VectorLength(v) < 200) {
						if (k && zc->followmate->client->zc.route_trace && (zc->followmate->client->zc.routeindex - 2) > 0 && (ent->svflags & SVF_MONSTER)) {
						...
						if (zc->followmate->client->zc.route_trace && (ent->svflags & SVF_MONSTER))
							zc->routeindex = zc->followmate->client->zc.routeindex;
						else {
							if (trace_priority < TRP_MOVEKEEP)
								zc->moveyaw = Get_yaw(v);
							if (trace_priority < TRP_ANGLEKEEP) {
	...
	if (ctf->value && !chedit->value) {
		...
		if (ent->client->resp.ctf_team == CTF_TEAM1) {
			...
			if (zc->ctfstate == CTFS_DEFENDER || zc->ctfstate == CTFS_CARRIER || zc->ctfstate == CTFS_SUPPORTER)
				j = FOR_FLAG1;
			else
				j = FOR_FLAG2;
		} else if (ent->client->resp.ctf_team == CTF_TEAM2) {
		...
		if (zc->route_trace) {
			if (Route[zc->routeindex].state < GRS_ITEMS && Route[zc->routeindex].linkpod[MAXLINKPOD - 1]) {
				...
				if (1 /*!Route[zc->routeindex].ent->union_ent*/) {
					...
					if (zc->routeindex + 1 < CurrentIndex) {
						...
						if ((!j || (v[2] - ent->s.origin[2]) > JumpMax) && it_ent->union_ent) {
							k = false;
							if ((it_ent->union_ent->s.origin[2] - ent->s.origin[2]) > JumpMax)
								k = true;
							...
							if (x < dist)
								dist = x;
							if (it_ent->nextthink > level.time)
								zc->rt_locktime = it_ent->nextthink + FRAMETIME * POD_LOCKFRAME;
							else
								zc->rt_locktime = level.time + FRAMETIME * POD_LOCKFRAME;
							if (trace_priority < TRP_MOVEKEEP)
								zc->moveyaw = yaw;
			...
			if (zc->routeindex < CurrentIndex) {
				...
				if (Route[zc->routeindex].state == GRS_PUSHBUTTON) {
					...
					if (rs_trace.fraction != 1.0 && rs_trace.ent) {
						if ((rs_trace.ent->health || rs_trace.ent->takedamage) && rs_trace.ent->classname[0] != 'p' && rs_trace.ent->classname[0] != 'b') {
				...
				if (k && !(ent->client->buttons & BUTTON_ATTACK)) {
					...
					if (!zc->first_target && it_ent->takedamage) {
						if (i == WEAP_GRENADES || i == WEAP_GRENADELAUNCHER || i == WEAP_ROCKETLAUNCHER || i == WEAP_PHALANX || i == WEAP_BFG) {
							item = Fdi_BLASTER; //FindItem("Blaster");
							item->use(ent, item);
				...
				if (Route[zc->routeindex].state == GRS_ONTRAIN && !zc->waterstate /*< WAS_IN*/
				  /*ent->groundentity*/) {
					...
					if ((trmin[2] - ent->s.origin[2]) > /*2*/ JumpMax && (v[2] - ent->s.origin[2]) > JumpMax && ent->waterlevel < 3) {
						zc->route_trace = false;
#ifdef _DEBUG
				...
				if (ent->client->ps.pmove.pm_flags & PMF_DUCKED)
					...
					if (zc->waterstate == WAS_IN)
						...
						if (v[0] == ent->s.origin[0] && v[1] == ent->s.origin[1])
							f1 = -300;
						else
							f1 = -(JumpMax + 64);
					} else
				...
				if (v[0] <= (ent->absmax[0] - yaw) && v[0] >= (ent->absmin[0] + yaw)) {
					if (v[1] <= (ent->absmax[1] - yaw) && v[1] >= (ent->absmin[1] + yaw)) {
						if ((v[2] <= (ent->absmax[2] - f1) && v[2] >= (ent->absmin[2] + f2)) || Route[zc->routeindex].state == GRS_ONROTATE
						  /*|| zc->waterstate == WAS_FLOAT*/) {
				...
				if (zc->routeindex < CurrentIndex && trace_priority && zc->second_target == NULL) {
					if (1 /*TraceX(ent,Route[zc->routeindex].Pt)*/) {
						...
						if (ent->groundentity /*|| ent->waterlevel ) &&  
							temppos[2] < 32 || zc->waterstate != WAS_IN)*/
							...
							if (!k) {
								...
								if (x < dist && fabs(yaw) < 20 /*JumpMax*/ && k) {
									...
									else if ((Route[zc->routeindex].state == GRS_ITEMS || Route[zc->routeindex].state == GRS_NORMAL) && !rs_trace.allsolid && !rs_trace.startsolid && HazardCheck(ent, v) && fabs(bottom) < 20 && i && !ent->waterlevel /*&& rs_trace.fraction == 1.0*/) {
										j = false;
										if (v[2] < ent->s.origin[2] && bottom < 0)
											j = true;
										else if (v[2] >= ent->s.origin[2] && bottom >= 0)
											j = true;
										if (j) {
									...
									} else if ((Route[zc->routeindex].state == GRS_ITEMS || Route[zc->routeindex].state == GRS_NORMAL) && fabs(bottom) < 20 && ent->waterlevel
									  /*&& !(zc->zcstate & STS_LADDERUP)*/) {
										j = false;
										if (v[2] < ent->s.origin[2] && bottom < 0)
											j = true;
							...
							if ((zc->routeindex - 1) >= 0 &&
							    (Route[zc->routeindex].state == GRS_ONPLAT || Route[zc->routeindex].state == GRS_ONTRAIN)) {
								Get_RouteOrigin(zc->routeindex - 1, v);
								if (fabs(v[2] - ent->s.origin[2]) <= JumpMax) {
									if (zc->waterstate < WAS_IN
									    /*&& Route[zc->routeindex].ent->trainteam == NULL*/
									    && Route[zc->routeindex].ent->nextthink > level.time)
										k = true;
								}
							}
							if (k && !(zc->zcstate & STS_WAITS)) {
								if ((zc->routeindex + 1) < CurrentIndex) {
									...
									if ((v[2] - ent->s.origin[2]) > JumpMax) {
										if ((Route[zc->routeindex].ent->union_ent->s.origin[2] - ent->s.origin[2]) > JumpMax
										  /*|| !TraceX(ent,v)*/) {
											...
											for (i = 1; i <= 3; i++) {
												if (zc->routeindex - i >= 0) {
													...
													if (zc->waitin_obj->absmax[0] < (v[0] + ent->mins[0]))
														k = true;
													else if (zc->waitin_obj->absmax[1] < (v[1] + ent->mins[1]))
														k = true;
													else if (zc->waitin_obj->absmin[0] > (v[0] + ent->maxs[0]))
														k = true;
													else if (zc->waitin_obj->absmin[1] > (v[1] + ent->maxs[1]))
														k = true;
													if (k)
														break;
												}
											...
											if (k)
												VectorCopy(v, zc->movtarget_pt);
											else
												Get_RouteOrigin(zc->routeindex - 1, zc->movtarget_pt);
	...
	if ((zc->zcstate & STS_WAITSMASK) && (!zc->waitin_obj || !zc->waitin_obj->inuse)) {
...
gi.bprintf(PRINT_HIGH,"aw shit!!\n");*/
				if (it_ent->pos1[2] > it_ent->pos2[2] && ((it_ent->moveinfo.state == PSTATE_UP && it_ent->velocity[2] > 0) || it_ent->moveinfo.state == PSTATE_BOTTOM)
				  /*&& it_ent->s.origin[2] != it_ent->s.old_origin[2]*/) {
					...
					if (zc->route_trace) {
						...
						if (Route[zc->routeindex].ent == zc->waitin_obj && Route[zc->routeindex].state == GRS_ONPLAT) {
			...
			//on train
			else if (Q_stricmp(it_ent->classname, "func_train") == 0 && it_ent->nextthink >= level.time && ((it_ent->s.origin[2] - it_ent->s.old_origin[2]) > 0 || zc->route_trace))
...
//						&& it_ent->trainteam == NULL
					...
					if ((it_ent->s.origin[2] - it_ent->s.old_origin[2]) > 0
					  /*|| (it_ent->s.origin[2] == it_ent->s.old_origin[2] && x == it_ent->s.origin[2])*/) {
						...
						zc->zcstate |= STS_W_ONTRAIN;
						zc->waitin_obj = it_ent;
					} else if ((it_ent->s.origin[2] - it_ent->s.old_origin[2]) > -2 && trace_priority && zc->second_target == NULL) {
	...
	//on plat
		   ...
		   (zc->zcstate & STS_W_ONPLAT) || (zc->zcstate & STS_W_COMEPLAT) || (zc->zcstate & STS_W_ONDOORUP) || (zc->zcstate & STS_W_ONDOORDWN)) &&
		 !(zc->zcstate & STS_W_DONT)) {
		k = false;
		//if door
		if (zc->zcstate & (STS_W_ONDOORUP | STS_W_ONDOORDWN)) {
			...
			if (zc->zcstate & STS_W_ONDOORUP) {
				if (zc->waitin_obj->moveinfo.state == PSTATE_UP || zc->waitin_obj->moveinfo.state == PSTATE_BOTTOM)
					k = true;
			}
			// down
			else {
				if (zc->waitin_obj->moveinfo.state == PSTATE_TOP || zc->waitin_obj->moveinfo.state == PSTATE_DOWN)
					k = true;
			}
		} else if (zc->zcstate & STS_W_COMEPLAT) {
			if (Route[zc->routeindex].state == GRS_ONTRAIN) {
				if (!TraceX(ent, /*zc->waitin_obj*/ Route[zc->routeindex].ent->union_ent->s.origin))
					k = true;
				if ((/*zc->waitin_obj*/ Route[zc->routeindex].ent->union_ent->s.origin[2] + 8 - ent->s.origin[2]) > JumpMax)
					k = true;
			} else {
				if ((zc->waitin_obj->union_ent->s.origin[2] - ent->s.origin[2]) > JumpMax)
					k = true;
			}
			...
			if (/*!k &&*/ zc->waitin_obj->moveinfo.state == PSTATE_UP || zc->waitin_obj->moveinfo.state == PSTATE_BOTTOM)
				k = true;
	...
	// on train
		...
		if (zc->route_trace) {
			...
			if (TraceX(ent, v)) {
				...
				zc->rt_locktime = level.time + FRAMETIME * POD_LOCKFRAME;
		} else if (j || !zc->waitin_obj ||
		           (zc->waitin_obj->s.origin[2] - zc->waitin_obj->s.old_origin[2]) <= 0)
			i = true;
	...
	//wait for door open
		...
		if (!zc->waitin_obj || !trace_priority || zc->waitin_obj->moveinfo.state == PSTATE_TOP
		  /*|| (zc->waitin_obj->moveinfo.state == PSTATE_DOWN)*/) {
			//gi.bprintf(PRINT_HIGH,"release %i %i\n",trace_priority,zc->waitin_obj->moveinfo.state);
			zc->zcstate &= ~STS_WAITS;
			zc->waitin_obj = NULL;
		} else if (zc->waitin_obj->moveinfo.state == PSTATE_BOTTOM || zc->waitin_obj->moveinfo.state == PSTATE_UP) {
...
*/
	...
	//rocket jump
	//	ent->client->weaponstate = WEAPON_READY;
	if (ent->groundentity && ent->client->weaponstate == WEAPON_READY && zc->second_target && trace_priority < TRP_ANGLEKEEP) {
		if ((zc->second_target->s.origin[2] - ent->s.origin[2]) > 100 && ent->health > 70 && ent->waterlevel <= 1) {
			j = false;
			v[0] = ent->s.origin[0];
			v[1] = ent->s.origin[1];
...
GOMOVE:
	...
	if (!ent->groundentity && !ent->waterlevel && !ent->client->zc.trapped) {
		...
		for (x = 0; x < 90; x += 10) {
			...
			if (i) // true || (i == 2 && ent->velocity > 0))
			{
				...
				//turbo
				if (!ent->waterlevel && ent->s.origin[2] > ent->s.old_origin[2] && zc->route_trace && !(zc->zcstate & STS_LADDERUP) && !(zc->zcstate & STS_SJMASK) && (zc->routeindex + 1) < CurrentIndex && ent->velocity[2] >= 100 && ent->velocity[2] < (100 + ent->gravity * sv_gravity->value * FRAMETIME)) {
					Get_RouteOrigin(zc->routeindex, v);
	...
	waterjumped = false;
	if (ent->groundentity || ent->waterlevel) {
		if (ent->groundentity && /*zc->waterstate == WAS_NONE*/ ent->waterlevel <= 0)
			k = 1;
		else if (ent->waterlevel) {
			k = 2;
			if (zc->route_trace) {
		...
		for (x = 0; x <= 180 && dist != 0; x += 10) {
			...
			if ((j = Bot_moveT(ent, yaw, temppos, dist, &bottom))) {
				//special
				if (x == 0 && /*bottom < 20 &&*/ !ent->waterlevel && !(ent->client->ps.pmove.pm_flags & PMF_DUCKED)) {
					if (zc->second_target) {
						...
						if ((v[2] - (ent->s.origin[2] + bottom)) > f2 ||
							...
							if ((v[2] - ent->s.origin[2]) <= JumpMax) {
								if (Route[zc->routeindex].state == GRS_ONTRAIN && zc->waterstate < WAS_IN)
									break;
								if (zc->routeindex > 0)
									if (Route[zc->routeindex - 1].state == GRS_ONTRAIN && Route[zc->routeindex - 1].ent == ent->groundentity)
										break;
							}
						} else if (ent->groundentity) {
							...
							if (!Q_stricmp(ent->groundentity->classname, "func_rotating")) {
								...
								if (!TraceX(ent, v) || !HazardCheck(ent, v))
									break;
								if (!BankCheck(ent, v))
									break;
								if (Bot_Fall(ent, temppos, dist)) {
				...
				if (/*((bottom > 20 && !ent->waterlevel) || (bottom > 0 && (ent->waterlevel == 2 || (ent->waterlevel == 1 && ent->groundentity == NULL))))*/
				  bottom > 20 && bottom <= f2 && j == true && k && !(ent->client->ps.pmove.pm_flags & PMF_DUCKED)) {
				...
				//dropable?1
					...
					if (ladderdrop && bottom < 0 && !zc->waterstate /*!ent->waterlevel*/) {
						if (Bot_moveW(ent, yaw, temppos, dist, &bottom)) {
							if (zc->second_target)
								iyaw = zc->second_target->s.origin[2] - ent->s.origin[2];
							else
								iyaw = -41;
		...
		if (x >= 70) {
			...
			/*			else if(!zc->route_trace && zc->first_target == NULL)
			{
				...
				if (x > 90 && ent->groundentity) {
					if (!Q_stricmp(ent->groundentity->classname, "func_train"))
						k = true;
				} else if (x > 90 && Route[zc->routeindex].state == GRS_ONTRAIN)
					k = true;
				if (k && trace_priority < TRP_ANGLEKEEP) {
		...
		if (/*zc->waterstate*/ ent->waterlevel && !waterjumped) {
			...
			if (zc->second_target != NULL) {
				...
				} else if (x > 0 && zc->waterstate == WAS_IN && !(ent->client->ps.pmove.pm_flags & PMF_DUCKED)) //アイテム上方
				{
					if (ent->velocity[2] < 0)
						ent->velocity[2] = 0;
					if (Bot_Watermove(ent, temppos, dist, x)) {
			...
			} else if ((ent->air_finished - FRAMETIME * 20) < level.time && zc->waterstate == WAS_IN) {
			...
			if ((!ent->groundentity && !zc->waterstate && k && ent->velocity[2] < 1) || (zc->waterstate == WAS_IN && (ent->client->ps.pmove.pm_flags & PMF_DUCKED))) {
		...
		for (j = i - 1; j >= 0; j--) {
			...
			if (trent->classname) {
				if (!Q_stricmp(trent->classname, "func_button")) {
					...
					it_ent = trent;
					break;
				} else if (!Q_stricmp(trent->classname, "func_door") || !Q_stricmp(trent->classname, "func_door_rotating")) {
				...
				for (i = maxclients->value + 1; i < globals.num_edicts; i++, e++) {
					...
					if (Q_stricmp(string, e->targetname) == 0) {
						...
						if (e->classname[0] == 't') {
							...
							} else if (!Q_stricmp(e->classname, "target_laser") || !Q_stricmp(e->classname, "target_mal_laser")) {
								if (e->spawnflags & 1) {
							...
							if (!Q_stricmp(e->classname, "func_door") || !Q_stricmp(e->classname, "func_door_rotating")
									...
									if (e->spawnflags & PDOOR_TOGGLE) {
										...
										if (f1 > 0) {
											...
											if (e->moveinfo.state == PSTATE_BOTTOM || e->moveinfo.state == PSTATE_UP) {
												if ((trent->s.origin[2] - ent->s.origin[2]) > 32)
													zc->second_target = NULL;
											} else if (e->moveinfo.state == PSTATE_TOP || e->moveinfo.state == PSTATE_DOWN) {
												if ((trent->s.origin[2] - ent->s.origin[2]) < -48)
													zc->second_target = NULL;
											}
										} else {
											k = true;
...
VCHCANSEL_L:
	...
	if (ent->client->ctf_grapple) {
		...
		if (ent->waterlevel > 2) {
			ent->velocity[0] *= 0.5;
			ent->velocity[1] *= 0.5;
		} else if (ent->waterlevel && !ent->groundentity && ent->velocity[2] < 0)
