...
void ThrowGib (edict_t *self, char *gibname, int damage, int type)
{
	...
	if (G_EntityUseNearEmergencyThreshold())
		return;
...
void target_string_use (edict_t *self, edict_t *other /* unused */, edict_t *activator /* unused */)
{
...
func_clock_format_counter (int type, int t, char *str, size_t str_sz)
{
	...
	switch (type) {
		case CLOCK_TYPE_S:
			...
			break;
		case CLOCK_TYPE_MS:
			Com_sprintf(str, str_sz, "%2i:%02i", t / 60, t % 60);
			break;
		case CLOCK_TYPE_HMS:
			Com_sprintf(str, str_sz, "%2i:%02i:%02i", t / 3600, (t - ((t / 3600) * 3600)) / 60, t % 60);
			break;
		default:
			*str = '\0';
	}
...
func_clock_format_currtime (char *str, size_t str_sz)
{
	...
	Com_sprintf(str, str_sz, "%2i:%02i:%02i", ltime->tm_hour, ltime->tm_min, ltime->tm_sec);
}
...
{
	return (e && e->inuse &&
		 e->classname && strcmp(e->classname, "target_string") == 0)
		 ? true
		 : false;
}
...
void func_clock_use (edict_t *self, edict_t *other /* unused */, edict_t *activator)
{
	if (!self) {
		return;
	}
