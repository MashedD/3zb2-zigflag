...
int SV_FlyMove (edict_t *ent, float time, int mask)
{
	...
	vec3_t primal_velocity, new_velocity;
	int i;
	trace_t trace;
	vec3_t end;
	float time_left;
...
qboolean SV_Push (edict_t *pusher, vec3_t move, vec3_t amove)
{
	...
	for (e = 1; e < globals.num_edicts; e++, check++) {
		...
		//ponko
		if (check->classname[0] == 'R' && (check->classname[6] == 'X' || check->classname[6] == '3'))
			continue;
...
void SV_Physics_Toss (edict_t *ent)
{
	...
	// SV_RunThink may call Bot_Think, which may respawn bot, which may reassign movetype
