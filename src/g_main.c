...
cvar_t *instagib;
cvar_t *chaingib;
cvar_t *chaingib_health_regen_delay;
cvar_t *chaingib_health_regen_rate;
cvar_t *chaingib_health_regen_cap;
cvar_t *chaingib_ammo_regen_delay;
cvar_t *chaingib_ammo_regen_rate;
cvar_t *chaingib_ammo_spawn;
cvar_t *chaingib_ammo_cap;
cvar_t *zigspawn;
cvar_t *zigkiller;
cvar_t *zigrapple;
cvar_t *zigintro;
...
void Get_NextMap ()
{
	...
	while (1) {
		...
		if (Buff[0] == 10 || Buff[0] == 13 || feof(fp))
			continue;
					...
					if (g_edicts[i].client->pers.inventory[ITEM_INDEX(zflag_item)]) {
						...
						if (next_fragadd < level.time) {
							if (flagholder != lastholder) {
								...
								if (killerflag->value && (level.time - flagholder->last_action_time) > (FRAMETIME * ZIGTICK * PENRATIO)) {
